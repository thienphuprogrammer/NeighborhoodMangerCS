...
namespace KhuPhoManager.Views.Forms
{
    public class ReadFileForm : Form
    {
        ...
        private async void LoadFile()
        {
            try
            {
                // Use Task.Run to execute the file loading operation asynchronously
                bool success = await System.Threading.Tasks.Task.Run(() => _controller.ReadFromFile(_filename));
                ...
                else
                {
                    progressBar.Style = ProgressBarStyle.Continuous;
                    progressBar.Value = 0;
                    statusLabel.Text = "Failed to load file. Check format and try again.";
